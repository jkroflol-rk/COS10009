...
class GameWindow < Gosu::Window
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      # boundary side
      if not @columns[cell_x][cell_y].east == nil
      ...
      if east_path == nil
        if not @columns[cell_x][cell_y].north == nil
        ...
        if not @columns[cell_x][cell_y].south == nil
        ...
        if not @columns[cell_x][cell_y].west == nil
