...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    while (column_index < x_cell_count)
      ...
      while (row_index < y_cell_count)
        if(column_index > 0)
          @columns[column_index-1][row_index].east = @columns[column_index][row_index]
        else
          @columns[column_index][row_index].west = nil
        end
        if(column_index < x_cell_count-1)
          @columns[column_index+1][row_index].west = @columns[column_index][row_index]
        else
          @columns[column_index][row_index].east = nil
        end
        if(row_index > 0)
          @columns[column_index][row_index-1].south = @columns[column_index][row_index]
        else
          @columns[column_index][row_index].north = nil
        end
        if(row_index < y_cell_count-1)
          @columns[column_index][row_index+1].north = @columns[column_index][row_index]
        else
    ...
    case id
      ...
      when Gosu::KB_L
        for i in 0..@columns.length-1
            for j in 0..@columns[0].length-1
              east = 0
              west = 0
              south = 0
              north = 0
              if @columns[i][j].north != nil
                north = 1
              end
              if @columns[i][j].south != nil
                south = 1
              end
              if @columns[i][j].west != nil
                west = 1
              end
              if @columns[i][j].east != nil
                east = 1
              end
                puts "Cell x: " + i.to_s + ", y: " + j.to_s + " north: " + north.to_s + " south: " + south.to_s + " east: " + east.to_s + " west: " + west.to_s
            end
            puts "---------- End of Column ----------"
        end
      end
  ...
  # if(cell[0]>0)
  #           @columns[cell[0]-1][cell[1]].east = @columns[cell[0]][cell[1]]
  #       else
  #         @columns[cell[0]-1][cell[1]].east = nil
  #       end
  #       if cell[0]<@columns.length-1
  #           @columns[cell[0]+1][cell[1]].west = @columns[cell[0]][cell[1]]
  #         else
  #           @columns[cell[0]-1][cell[1]].west = nil
  #       end
  #       if cell[1]>0
  #           @columns[cell[0]][cell[1]-1].south = @columns[cell[0]][cell[1]]
  #         else
  #           @columns[cell[0]-1][cell[1]].south = nil
  #       end
  #       if cell[1]<@columns[0].length-1
  #           @columns[cell[0]][cell[1]+1].north = @columns[cell[0]][cell[1]]
  #         else
  #           @columns[cell[0]-1][cell[1]].north = nil
  #       end

  def cursor_on_cell(col, row)
    if(mouse_x > row*CELL_DIM && mouse_x < (row+1)*CELL_DIM && mouse_y > col*CELL_DIM && mouse_y < (col+1)*CELL_DIM)
        return true
    else
        return false
    end
      ...
      while (row_index < y_cell_count)
        ...
        if (@columns[column_index][row_index].vacant)
          color = Gosu::Color::YELLOW
            ...
            if(cursor_on_cell(row_index, column_index))
