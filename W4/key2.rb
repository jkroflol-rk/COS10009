  ...
  def initialize
    ...
    while column_index < x_cell_count
          ...
          while row_index < y_cell_count
                ...
                east  = 1
                west = 1
                @columns[column_index][row_index].north = north
                @columns[column_index][row_index].south = south
                @columns[column_index][row_index].east = east
                @columns[column_index][row_index].west = west
                row_index += 1
             end
          column_index += 1
       end
       ...
       column_index =0
       while column_index< x_cell_count 
        @columns[column_index][0].north =0
        @columns[column_index][9].south =0
        column_index+=1
       end

       row_index =0
       while row_index<y_cell_count
        @columns[0][row_index].west =0
        @columns[9][row_index].east =0
        row_index+=1
       end
       ...
       while column_index< x_cell_count
        row_index=0
        while row_index< y_cell_count
          puts "Cell x: " + column_index.to_s + ", y: " + row_index.to_s + " north:" + @columns[column_index][row_index].north.to_s + " south: " + @columns[column_index][row_index].south.to_s + " east: " + @columns[column_index][row_index].east.to_s + " west: " + @columns[column_index][row_index].west.to_s
          row_index += 1
        end
        puts '------------ End of Column ------------'
        column_index+=1
